			...
			<a id="accordion-<?php echo esc_attr( $key ) ?>-label" class="accordion-title plain<?php echo $is_open ? ' active' : ''; ?>" aria-expanded="<?php echo $is_open ? 'true' : 'false'; ?>" aria-controls="accordion-<?php echo esc_attr( $key ) ?>-content" href="<?php echo esc_url( '#accordion-item-' . $key ); ?>">
				<?php 
					if ($key == 'reviews') {
						...
						echo apply_filters( 'woocommerce_product_' . $key . '_tab_title', $product_tab['title'], $key ); 
					}
					?>
...
<?php endif; ?>
<div class="content_product_check">
    <b>Chú ý:</b><br />
    <p>* Khuyến mãi tặng thêm quà cho một sản phẩm chỉ có thể được
    áp dụng khi chọn từ các tùy chọn (nếu mua riêng lẻ, không thể tặng thêm quà)</p>
		<p>* Không thể trả lại một phần sau khi đặt hàng các mặt hàng khuýen mãi hoặc các mặt hàng theo gói
    (chỉ có thể trả lại toàn bộ các mặt hàng theo gói(không thể trả lại theo sản phẩm))</p>
</div>
